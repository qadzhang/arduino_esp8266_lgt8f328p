...
IPAddress PGIP(182, 162, 130, 229); // your PostgreSQL server IP

const char ssid[] = "fdgfdf"; //  your network SSID (name)
const char pass[] = "xfdgom"; // your network password

const char user[] = "eer2p";                  // your database user
const char password[] = "gfdfddf"; // your database password
const char dbname[] = "e3fs4p";                // your database name
int port = 7896;                            // your database port
...
String convertFloatToString(float temperature) { // begin function
...
void doPg() {
	// ESP.wdtFeed();
	String query_sql =
			"insert  into esp_recodes values (now(),  inet_client_addr() ,upper('"
					+ MAC_array_STAs + "'),'BMP180','{ \"temperature\":    "
					+ Temperatures + ", \"barometer\":" + Pressures + "}')  ";
	...
	int rc;
	if (!pg_status) {
		conn.setDbLogin(PGIP, user, password, dbname, "utf8", port);
		pg_status = 1;
		// ESP.wdtFeed();
		//   return;
		// Serial.println(pg_status);
