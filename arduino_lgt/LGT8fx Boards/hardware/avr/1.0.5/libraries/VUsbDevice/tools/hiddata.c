...
int bytesSent, reportId = buffer[0];
    ...
    if(!usesReportIDs){
        p++;   /* skip dummy report ID */
        len--;
    }
    bytesSent = usb_control_msg((void *)device, USB_TYPE_CLASS | USB_RECIP_DEVICE | USB_ENDPOINT_OUT, USBRQ_HID_SET_REPORT, USB_HID_REPORT_TYPE_FEATURE << 8 | (reportId & 0xff), 0, p, len, 5000);
    if(bytesSent != len){
        if(bytesSent < 0)
